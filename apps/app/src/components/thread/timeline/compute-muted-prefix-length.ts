 ...
 * renderer can style it muted. We never extract data from the prefix — only
 * locate its boundary based on the leading `[bb` marker. Trailing whitespace
 * after `]` is absorbed into the muted region so block (`\n\n`) and inline
 * (` `) writer-side separators render identically: header on one line, body
 * directly below, with no blank gap.
 *
