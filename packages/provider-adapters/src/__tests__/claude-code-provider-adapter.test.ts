...
describe("claude-code provider adapter", () => {
  ...
  it("buildCommand thread/start includes managerMode for manager threads", () => {
    const adapter = createClaudeCodeProviderAdapter();
    const cmd = adapter.buildCommand({
      ...
      context: { projectId: "proj-1", threadId: "bb-thread-1" },
    });
    expect(cmd?.params).toMatchObject({ managerMode: true });
  });

  it("buildCommand thread/start routes threadId from context", () => {
    const adapter = createClaudeCodeProviderAdapter();
    const cmd = adapter.buildCommand({
      type: "thread/start",
      threadId: "bb-thread-1",
      req: { projectId: "proj-1", input: [{ type: "text", text: "hello" }] },
      context: { projectId: "proj-1", threadId: "bb-thread-1" },
    });
