...
export class WorkspaceStatusWatcher {
  ...
  private async startAsync(): Promise<void> {
    if (!(await detectGitRepo(this.args.cwd))) {
      return;
    }
    if (this.disposed) {
      return;
  ...
  private startMetadataWatchSubscriptions(): void {
    void this.startMetadataWatchSubscriptionsAsync();
  }

  private async startMetadataWatchSubscriptionsAsync(): Promise<void> {
    try {
