...
import type { IWorkspace } from "../src/provision.js";
...
describe("provisionWorkspace", () => {
  describe("unmanaged", () => {
    it("provisions an unmanaged git repo and discovers properties", async () => {
      ...
      expect(await ws.currentBranch()).toBe("main");
    });
  ...
  describe("managed-worktree", () => {
    it("provisions a worktree and returns IWorkspace", async () => {
      const repoPath = await initRepo();
      ...
      expect(ws.isGitRepo).toBe(true);
      expect(ws.isWorktree).toBe(true);
      expect(await ws.currentBranch()).toBe("bb/env-test");
    });
    ...
    it("rolls back on setup script failure", async () => {
      const repoPath = await initRepo({ setupScript: "exit 1\n" });
  ...
  describe("managed-clone", () => {
    it("provisions a clone and returns IWorkspace", async () => {
      const repoPath = await initRepo();
      ...
      expect(ws.isGitRepo).toBe(true);
      expect(ws.isWorktree).toBe(false);
      expect(await ws.currentBranch()).toBe("bb/clone-branch");
    });
  ...
  describe("IWorkspace promote/demote", () => {
    it("promotes and demotes round-trip", async () => {
      ...
      expect(await primary.currentBranch()).toBe("bb/env-rt");
      expect(await env.currentBranch()).toBeNull();
