...
CREATE TABLE `__new_threads` (
	...
	`title` text,
	`status` text DEFAULT 'created' NOT NULL CHECK(`status` IN ('created', 'provisioning', 'provisioned', 'provisioning_failed', 'error', 'idle', 'active')),
	`environment_id` text,
...
SELECT
	...
	`title`,
	CASE
		WHEN `status` IN ('created', 'provisioning', 'provisioned', 'provisioning_failed', 'error', 'idle', 'active') THEN `status`
		ELSE 'created'
	END,
	`environment_id`,
	`environment_record`,
