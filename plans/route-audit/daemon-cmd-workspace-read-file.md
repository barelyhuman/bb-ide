...
3. **No current product caller.** The public thread workspace read routes were removed when the manager viewer moved to durable thread-storage routes backed by `host.read_file`.
...
|---|---|---|
| None (current product routes) | — | `GET /api/v1/threads/:id/thread-storage/file` was removed once the manager viewer moved to durable thread-storage routes |
...
- March 30, 2026 investigation:
  ...
  2. Thread storage browsing now uses `host.read_file` against the durable thread storage root `<dataDir>/thread-storage/<threadId>`.
