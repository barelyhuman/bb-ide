...
5. `createDaemonFileContentResponse(...)`:
   ...
   - sets `Content-Type`, `x-bb-content-encoding`, and `x-bb-size-bytes`
...
1. **Raw response body, not JSON.** The app must inspect headers and body bytes to decide whether to render text, an image, or an unsupported preview state.
...
|---|---|---|
| `getManagerWorkspaceFilePreview` | `apps/app/src/lib/api.ts` | Fetches raw bytes and headers for manager workspace previews |
