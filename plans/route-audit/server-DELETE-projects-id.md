...
1. `deleteProject` does not call `notifier.notifyProject(...)` or `notifier.notifySystem(...)` after deletion. Connected clients won't get a real-time update that the project was removed. Compare with `createProject` which does notify.
