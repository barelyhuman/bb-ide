...
| Q34 | confirmed | identity key differs: host excludes `threadId`, docker includes it (`host-environment-agent.ts:60-65`, `docker-environment-agent.ts:63-67`). | confirmed — host key: `[projectId, environmentId, workspaceRootPath]`; docker key: `[projectId, threadId, environmentId, workspaceRootPath]`; two threads sharing docker env get separate agent records. | deferred — both host and docker keys may need review: host excludes threadId (enables shared reuse), docker includes it (could cause duplicate agents in shared containers). Needs product-level decision on shared environment agent ownership. |
