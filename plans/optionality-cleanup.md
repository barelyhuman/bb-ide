...
- [x] Plumb `turn.steer.options` through daemon dispatch, runtime, and provider adapters. Do not leave it accepted-and-ignored.
...
- [x] Fix `turn.steer.options` behavior so steer-time execution overrides are honored end-to-end and go through the same server helper that fills in defaults for other turn entry points.
