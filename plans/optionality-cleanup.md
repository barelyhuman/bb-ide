...
- [ ] Make `CreateThreadRequest.input` required in `@bb/server-contract`.
...
- [ ] Make `model` required in the public create-thread API. Keep `serviceTier`, `reasoningLevel`, and `sandboxMode` optional on create-thread, with server-owned defaults.
...
- [ ] Make CLI `thread spawn --prompt` required so it cannot create an idle thread shell.
...
- [ ] Make `SendMessageRequest.mode` required and update the app and CLI to always send explicit mode intent.
...
- [ ] Make `ArchiveThreadRequest.force` required and update all callers to send `false` or `true` explicitly.
...
- [ ] Delete `includeUnstaged` for plain commit actions and make commit always include all workspace changes.
...
- [ ] Make the public diff API require `selection`, and make the daemon contract require it too.
...
- [ ] Make the public status and diff APIs require `mergeBaseBranch`, and make the daemon commands require it too.
...
- [ ] Delete `workspace.checkpoint.remoteName` and make checkpoint always use `origin`.
